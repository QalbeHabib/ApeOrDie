...
(async () => {
  ...
  console.log(
    `Attempting to accept authority as: ${pendingAuthorityWallet.publicKey.toBase58()}`
  );

  // Instantiate the program using the IDL and programId.
  const program = anchor.workspace.ApeOrDie as Program<ApeOrDie>;
  console.log("Using ApeOrDie program ID:", program.programId.toBase58());
  ...
  try {
    ...
    console.log(
      `Current on-chain authority: ${configState.authority.toBase58()}`
    );
    console.log(
      `Current on-chain pending authority: ${configState.pendingAuthority.toBase58()}`
    );

    if (configState.pendingAuthority.equals(web3.PublicKey.default)) {
      console.error(
        "Error: No pending authority set in the config account. Run nominate_authority first."
      );
      return;
    }

    if (
      !configState.pendingAuthority.equals(pendingAuthorityWallet.publicKey)
    ) {
      console.error(
        `Error: The configured wallet (${pendingAuthorityWallet.publicKey.toBase58()}) does not match the pending authority (${configState.pendingAuthority.toBase58()}) set in the config account.`
      );
      return;
    }
    ...
    console.log(
      `New on-chain authority: ${updatedConfigState.authority.toBase58()}`
    );
    console.log(
      `New on-chain pending authority: ${updatedConfigState.pendingAuthority.toBase58()} (should be default)`
    );
    ...
    if (updatedConfigState.authority.equals(pendingAuthorityWallet.publicKey)) {
      ...
      console.error(
        "Error: Authority was not updated correctly after transaction."
      );
    }
  } catch (err) {
